...
`OverlayPanel` is recreated on config change (no runtime style toggling — old panel disposed, new panel constructed from fresh config values).
...
**ImrdyPalette**: `src/Imrdy.Windows/Theme/ImrdyPalette.cs` — shared theme helper extracted from `HoverDashboardFormBase`. Provides palette colors and `ApplyMica` / `ApplyRoundedRegion` helpers; consumed by `HoverDashboardFormBase`, `SessionDashboardForm`, `WorkspaceDashboardForm`, and `OverlayPanel`.

**Overlay interactivity**: The `WM_NCHITTEST` gap click-through policy was dropped (Decision 11). `OverlayPanel` has no `WM_NCHITTEST` override — inter-chip gaps are opaque panel chrome and clicks there are simple no-ops. Input is handled only via `OnMouseDown` (left-click → activate) and `OnMouseUp` (right-click → context menu). `ScreenToClientPoint` P/Invoke is still used for DPI-correct screen→client conversion in the hover-highlight poll and hit-testing (`Bounds`-subtraction is wrong above 100% scale). The passive (fully click-through) variant is removed — use `overlay.enabled: false` to suppress the overlay entirely.
