...
- [Overlay Interactivity](overlay-interactivity.md) — Overlay split into Passive/Interactive classes; context menus dispatched via ISessionInteractionRouter + MenuAnchor.AtControl(owner, location) — vanilla WinForms, no P/Invoke band-aids
...
- [Hover Dashboard State Machine](hover-dashboard-state-machine.md) — Hover preview controller must distinguish cursor traversal (grace corridor) from user commitment (click action) — add SurfaceInteracted event for immediate dismiss; post-interaction cooldown prevents ghost re-show when cursor stays in hover bounds after click
