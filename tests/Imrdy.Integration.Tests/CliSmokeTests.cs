...
public class CliSmokeTests : IDisposable
{
    ...
    {
        var (exitCode, stdout, _) = await _cli.RunAsync("--version", workingDirectory: _temp.Path);
    ...
    {
        var (exitCode, stdout, _) = await _cli.RunAsync("--help", workingDirectory: _temp.Path);
    ...
    {
        var (exitCode, _, _) = await _cli.RunAsync("status", workingDirectory: _temp.Path);
    ...
    {
        var (exitCode, _, _) = await _cli.RunAsync("config show", workingDirectory: _temp.Path);
